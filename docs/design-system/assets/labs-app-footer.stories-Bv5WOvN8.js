import"./labs-footer-C7naslGc.js";import"./labs-settings-overlay-C_Oo8S_x.js";import"./labs-button-DHQuKs1y.js";import"./labs-icon-CZw9JeVg.js";import"./labs-theme-toggle-button-Cj9_1PaE.js";import"./button-configs-DUKrH4Iw.js";class s extends HTMLElement{constructor(){super()}static get observedAttributes(){return["add-label","variant"]}attributeChangedCallback(){this.render()}connectedCallback(){this.render(),this.setupEventListeners()}setupEventListeners(){Promise.all([customElements.whenDefined("labs-footer"),customElements.whenDefined("labs-settings-overlay")]).then(()=>{setTimeout(()=>{const t=this.querySelector("labs-footer"),e=this.querySelector("labs-settings-overlay");t&&e?t.addEventListener("settings-click",()=>{typeof e.open=="function"?e.open():console.error("labs-settings-overlay: open method not available")}):console.warn("labs-app-footer: Child components not found",{footer:t,overlay:e})},10)}).catch(t=>{console.error("labs-app-footer: Error waiting for component definitions",t)})}render(){const t=this.getAttribute("add-label")||"+ Add",e=this.getAttribute("variant")||"default";this.innerHTML=`
      <!-- Footer Component -->
      <labs-footer 
        add-label="${t}" 
        ${e!=="default"?`variant="${e}"`:""}
