const i = {
  ...
  t = () => {
    ...
    (n.style.display = "grid"),
      (n.style.gridTemplateColumns = "repeat(auto-fit, minmax(180px, 1fr))"),
      (n.style.gap = "1rem");
    function s(r) {
      if (
        ((r = r.trim()),
        ...
        const o = r.match(/\d+/g).map(Number);
        return 0.299 * o[0] + 0.587 * o[1] + 0.114 * o[2] > 186
          ? "#222"
          : "#fff";
      }
      if (
        ...
        const o = parseInt(r.substr(0, 2), 16),
          e = parseInt(r.substr(2, 2), 16),
          a = parseInt(r.substr(4, 2), 16);
        return 0.299 * o + 0.587 * e + 0.114 * a > 186 ? "#222" : "#fff";
      }
      return "#222";
    }
    return (
      c.forEach((r) => {
        const o = document.createElement("div"),
          e = getComputedStyle(document.documentElement)
            .getPropertyValue(`--${r}`)
            .trim();
        (o.style.background = `var(--${r})`),
          (o.style.color = s(e)),
          (o.style.padding = "1.5rem"),
          (o.style.borderRadius = "1rem"),
          (o.style.textAlign = "center"),
          (o.innerHTML = `<strong>--${r}</strong><br/>${e}`),
          n.appendChild(o);
      }),
      n
    );
...
const m = ["Colors"];
export { t as Colors, m as __namedExportsOrder, i as default };
