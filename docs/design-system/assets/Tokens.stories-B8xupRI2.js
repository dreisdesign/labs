const p = {
    ...
    parameters: {
      docs: {
        description: {
          component:
            "All design tokens (colors, typography, spacing) used in the Labs Design System.",
        },
      },
    },
  },
  a = () => {
    ...
    function r(e) {
      if (
        ...
        return 0.299 * n[0] + 0.587 * n[1] + 0.114 * n[2] > 186
          ? "#222"
          : "#fff";
      }
      if (
        ...
        return 0.299 * n + 0.587 * s + 0.114 * i > 186 ? "#222" : "#fff";
      }
      return "#222";
    }
    return (
      o.forEach((e) => {
        const n = document.createElement("div"),
          s = getComputedStyle(document.documentElement)
            .getPropertyValue(`--${e}`)
            .trim();
        (n.style.background = `var(--${e})`),
          (n.style.color = r(s)),
          (n.style.padding = "1.5rem"),
          (n.style.borderRadius = "1rem"),
          (n.style.textAlign = "center"),
          (n.innerHTML = `<strong>--${e}</strong><br/>${s}`),
          t.appendChild(n);
      }),
      t
    );
...
const c = () => {
  ...
  return (
    ...
    o.forEach((r) => {
      ...
      (e.style.fontFamily =
        r === "font-family-base" ? `var(--${r})` : "inherit"),
        (e.style.fontSize = r.startsWith("font-size") ? `var(--${r})` : "1rem"),
        (e.style.fontWeight = r.startsWith("font-weight")
          ? `var(--${r})`
          : "400"),
        (e.innerHTML = `<strong>--${r}</strong>: <span style='font-family:var(--font-family-base);'>${getComputedStyle(document.documentElement).getPropertyValue(`--${r}`)}</span>`),
        t.appendChild(e);
    }),
    t
  );
};
c.storyName = "Typography";
const l = () => {
  const o = ["space-xs", "space-sm", "space-md", "space-lg", "space-xl"],
    t = document.createElement("div");
  return (
    (t.style.display = "flex"),
    (t.style.flexDirection = "column"),
    ...
    o.forEach((r) => {
      ...
      (e.style.height = `var(--${r})`),
        ...
        (e.innerHTML = `<strong>--${r}</strong> <span style="margin-left:0.5rem; color:#666;">${getComputedStyle(document.documentElement).getPropertyValue(`--${r}`)}</span>`),
        t.appendChild(e);
    }),
    t
  );
...
const d = ["Colors", "Typography", "Spacing"];
export {
  a as Colors,
  l as Spacing,
  c as Typography,
