class c extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._open=!1,this._menuId=`labs-dropdown-menu-${Math.random().toString(36).slice(2,8)}`,this.render()}static get observedAttributes(){return["archived","restored","open","only"]}attributeChangedCallback(t){t==="open"&&(this._open=this.hasAttribute("open")),this.render()}get open(){return this._open}set open(t){!!t?this.setAttribute("open",""):this.removeAttribute("open")}connectedCallback(){document.addEventListener("click",this._outsideClick=t=>{if(!this._open)return;const e=t.composedPath?t.composedPath():t.path||[];!e.includes(this)&&!e.includes(this.shadowRoot)&&(!this._portaledMenu||!e.includes(this._portaledMenu))&&this._close()})}disconnectedCallback(){document.removeEventListener("click",this._outsideClick),this._cleanupPortaledMenu()}render(){this.shadowRoot.innerHTML=`
    ...
    `,this._createPortaledMenu();const t=this.shadowRoot.getElementById("toggleBtn");t&&(t.addEventListener("click",o=>{o.stopPropagation(),this._toggle()}),t.addEventListener("keydown",o=>this._onToggleKeyDown(o))),this._open?(this._portaledMenu&&(this._portaledMenu.style.display="block",this._portaledMenu.setAttribute("aria-hidden","false"),this._positionPortaledMenu()),this.dispatchEvent(new CustomEvent("open",{bubbles:!0,composed:!0}))):(this._portaledMenu&&(this._portaledMenu.style.display="none",this._portaledMenu.setAttribute("aria-hidden","true")),this.dispatchEvent(new CustomEvent("close",{bubbles:!0,composed:!0})));const e=this.shadowRoot.getElementById("toggleBtn");e&&e.setAttribute("aria-expanded",String(this._open))}_getMenuItems(){const t=this._portaledMenu||document.getElementById(this._menuId);return t?Array.from(t.querySelectorAll("labs-button")):[]}_focusItemAt(t){const e=this._getMenuItems();if(!e.length)return;const o=(t+e.length)%e.length,n=e[o];if(n){n.hasAttribute("tabindex")||n.setAttribute("tabindex","0");try{n.focus()}catch{}this._currentIndex=o}}_focusFirstItem(){this._focusItemAt(0)}_focusLastItem(){const t=this._getMenuItems();this._focusItemAt(t.length-1)}_onToggleKeyDown(t){const e=t.key;e==="Enter"||e===" "||e==="Spacebar"?(t.preventDefault(),t.stopPropagation(),this.open=!0,this._focusFirstItem()):e==="ArrowDown"?(t.preventDefault(),t.stopPropagation(),this.open=!0,this._focusFirstItem()):e==="ArrowUp"&&(t.preventDefault(),t.stopPropagation(),this.open=!0,this._focusLastItem())}_onMenuKeyDown(t){const e=t.key;if(this._getMenuItems().length)if(e==="ArrowDown")t.preventDefault(),t.stopPropagation(),this._focusItemAt((this._currentIndex||0)+1);else if(e==="ArrowUp")t.preventDefault(),t.stopPropagation(),this._focusItemAt((this._currentIndex||0)-1);else if(e==="Home")t.preventDefault(),t.stopPropagation(),this._focusFirstItem();else if(e==="End")t.preventDefault(),t.stopPropagation(),this._focusLastItem();else if(e==="Escape"){t.preventDefault(),t.stopPropagation(),this._close();const n=this.shadowRoot.getElementById("toggleBtn");n&&n.focus()}else e==="Tab"&&this._close()}_toggle(){this.open=!this._open}_close(){this.open=!1}_createPortaledMenu(){let t=document.getElementById("labs-dropdown-portal");t||(t=document.createElement("div"),t.id="labs-dropdown-portal",t.style.cssText=`
                position: fixed;
                top: 0;
                left: 0;
                pointer-events: none;
            ...
            pointer-events: auto;
        `;const s=(this.getAttribute("only")||"").split(",").map(i=>i.trim().toLowerCase()).filter(Boolean),r=s.length===0||s.includes("archive"),l=s.length===0||s.includes("delete");o.innerHTML=`
            <style>
            ...
            ${r?`<labs-button id="archiveBtn" variant="secondary" size="small" fullwidth role="menuitem" tabindex="-1">
                <labs-icon slot="icon-left" name="archive" width="20" height="20"></labs-icon>
                Archive
            </labs-button>`:""}
            ${l?`<labs-button id="deleteBtn" variant="destructive" size="small" fullwidth role="menuitem" tabindex="-1">
                <labs-icon slot="icon-left" name="delete_forever" width="20" height="20"></labs-icon>
                Delete
            </labs-button>`:""}
        `,t.appendChild(o),this._portaledMenu=o,this._wireMenuEvents(o)}_wireMenuEvents(t){const e=t.querySelector("#archiveBtn");e&&e.addEventListener("click",n=>{n.preventDefault(),n.stopPropagation(),this.dispatchEvent(new CustomEvent("archive",{bubbles:!0,composed:!0})),this._close()});const o=t.querySelector("#deleteBtn");o&&o.addEventListener("click",n=>{n.preventDefault(),n.stopPropagation(),this.dispatchEvent(new CustomEvent("remove",{bubbles:!0,composed:!0})),this._close()}),t.addEventListener("keydown",n=>this._onMenuKeyDown(n))}_positionPortaledMenu(){if(!this._portaledMenu)return;const t=this.shadowRoot.getElementById("toggleBtn");if(!t)return;const e=t.getBoundingClientRect(),o=this._portaledMenu,n=o.style.display;o.style.display="block";const s=o.getBoundingClientRect();o.style.display=n;const r=window.innerWidth,i=window.innerHeight-e.bottom,a=e.top,d=Math.min(Math.max(0,e.right-s.width),r-s.width),u=i>=s.height||i>=a?e.bottom:e.top-s.height;o.style.left=`${d}px`,o.style.top=`${u}px`,this._repositionHandler||(this._repositionHandler=()=>{try{this._positionPortaledMenu()}catch{}},window.addEventListener("resize",this._repositionHandler,{passive:!0}),window.addEventListener("scroll",this._repositionHandler,{passive:!0}))}_cleanupPortaledMenu(){this._portaledMenu&&(this._portaledMenu.remove(),this._portaledMenu=null),this._repositionHandler&&(window.removeEventListener("resize",this._repositionHandler),window.removeEventListener("scroll",this._repositionHandler),this._repositionHandler=null)}}customElements.get("labs-dropdown")||customElements.define("labs-dropdown",c);
