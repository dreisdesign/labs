import"./labs-icon-BWc3JKvc.js";class g extends HTMLElement{static get observedAttributes(){return["variant","icon-left","icon-right","checkmark","label","size"]}constructor(){super(),this.attachShadow({mode:"open"}),this.animating=!1,this.handleClick=this.handleClick.bind(this)}connectedCallback(){console.log("[LabsButton] LOCAL COMPONENT LOADED - DEMO TEST"),this.render(),this.themeObserver=new MutationObserver(a=>{a.forEach(t=>{t.type==="attributes"&&t.attributeName==="class"&&(t.target.classList.contains("theme-dark")||t.target.classList.contains("theme-light"))&&this.render()})}),this.themeObserver.observe(document.body,{attributes:!0,attributeFilter:["class"]})}disconnectedCallback(){const a=this.shadowRoot.querySelector("button");a&&a.removeEventListener("click",this.handleClick),this.themeObserver&&this.themeObserver.disconnect()}attributeChangedCallback(){console.log("[LabsButton] attributeChangedCallback:",{variant:this.getAttribute("variant"),iconLeft:this.getAttribute("icon-left"),iconRight:this.getAttribute("icon-right"),checkmark:this.getAttribute("checkmark"),label:this.getAttribute("label"),iconcolor:this.getAttribute("iconcolor"),size:this.getAttribute("size")}),this.render()}handleClick(a){const t=this.shadowRoot.querySelector("button");if(t.setAttribute("data-active","true"),setTimeout(()=>{t.removeAttribute("data-active")},200),this.hasAttribute("checkmark")){if(this.animating)return;this.animating=!0,t.classList.remove("checkmark-animation"),t.offsetWidth,t.classList.add("checkmark-animation"),setTimeout(()=>{t.classList.remove("checkmark-animation"),this.animating=!1},800)}this.dispatchEvent(new CustomEvent("labs-click",{bubbles:!0}))}render(){const a=this.getAttribute("label")||"",t=a.toLowerCase(),c=this.getAttribute("variant")||"primary",n=this.getAttribute("size")||"lg";let r=this.getAttribute("iconcolor")||"",l="";const i=s=>s?s.replace(/\.(svg|png|jpg|jpeg)$/i,"").replace(/--fill|--outline|--regular|--solid/gi,"").replace(/-/g,"_"):"",e=i(this.getAttribute("icon-left"));let o=i(this.getAttribute("icon-right"));!o&&this.hasAttribute("default-icon-right")&&(o="settings");const b=this.hasAttribute("checkmark"),u=t.replace(/\s/g,""),d="",h=n==="sm"||!a&&(e||o)?"icon":n==="lg"?"large":"";this.shadowRoot.innerHTML=`
      <style>
        :host { 
        ...
        .labs-icon img {
          filter: ${r==="#fff"||r.toLowerCase()==="white"?"brightness(0) invert(1)":r?"hue-rotate(0deg) saturate(0) brightness(0) invert(1)":"none"};
        }
      ...
      <button class="labs-button ${c} ${h}" part="button"
        data-buttontype="${u}"
        style="--icon-active-color: ${l}; ${d}">
  ${e?`<labs-icon class="labs-icon" name="${e}"${r?` color="${r}"`:""}></labs-icon>`:""}
  <span class="labs-label">${a}</span>
  ${o?`<labs-icon class="labs-icon" name="${o}" color="${r}"></labs-icon>`:""}
  ${b?`<span class="labs-checkmark"><labs-icon name="check" class="labs-icon" color="${r||"white"}"></labs-icon></span>`:""}
      </button>
    `,this.shadowRoot.querySelector("button").addEventListener("click",this.handleClick)}}customElements.define("labs-button",g);
