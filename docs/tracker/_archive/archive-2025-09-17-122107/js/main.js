...
function isSameGroup(date1, date2) {
  if (isTestingMode) {
    ...
    return (
      ...
      Math.floor(date1.getSeconds() / 10) ===
        Math.floor(date1.getSeconds() / 10)
    );
  }
...
function initRefreshAppButton() {
  ...
  const versionSuccessMessage = document.getElementById(
...
function showCommentOverlay(entryId) {
  ...
  const commentOverlayTitle = document.querySelector(
    "#commentOverlay .overlay-header h2",
  );
  ...
  setTimeout(
    ...
    isMobileDevice() ? 150 : 50,
  ); // Longer delay for mobile, 50ms for desktop
}
...
function deleteComment() {
  ...
  if (currentEditingEntry && currentEditingEntry.comment) {
    ...
    const entryIndex = trackedEntries.findIndex(
      (entry) => entry.date === currentEditingEntry.date,
    );
    if (entryIndex !== -1) {
...
function updateVisibleEntryOpacities() {
  requestAnimationFrame(() => {
    ...
    const allEntries = Array.from(
      timestampList.querySelectorAll(".time-entry"),
    );
    ...
    const opacityStep = 0.2; // Each subsequent entry decreases by 5%
      ...
      visibleEntries.forEach((entry, index) => {
        ...
        if (index === 0) {
          ...
          if (visibleEntries.length >= 16) {
            ...
            targetOpacity =
              secondOpacity - opacityRange * (position / totalRemaining);
          } else {
  ...
  groups.forEach((group, groupIdx) => {
    ...
    group.entries.forEach((entry, entryIdx) => {
      ...
      entryDiv.addEventListener("click", () => {
        ...
        const isCurrentlySwipingAttr =
          entryDiv.getAttribute("data-swiping") === "true";
        const isSwipingFunc =
          typeof window.isSignificantSwipeDetected === "function" &&
          window.isSignificantSwipeDetected();
...
function backupCurrentData() {
  ...
  console.log(
    "Data backup complete. Total count:",
    backupTotalCount,
    "Entries:",
    backupTrackedEntries.length,
    "Label:",
    backupLabelText,
  );
}
...
function playFLIPAnimation(prevRects) {
  const newEls = Array.from(
    timestampList.querySelectorAll(".date-header, .time-entry"),
  );
  newEls.forEach((el) => {
...
undoButton.addEventListener("click", () => {
  ...
  if (deletedCommentEntry && deletedCommentText) {
    ...
    const entryIndex = trackedEntries.findIndex(
      (entry) => entry.date === deletedCommentEntry.date,
    );
    if (entryIndex !== -1) {
...
function hideUndoButton() {
  ...
  if (resetPending) {
    console.log(
      "Reset was pending, setting resetPending to false in hideUndoButton.",
    );
    resetPending = false;
  }
}
...
trackButton.addEventListener("click", () => {
  ...
  prevEls.forEach((el) => {
    ...
    if (key) {
      // Ensure key is not null/undefined
  ...
  if (typeof window.reinitializeDeleteFunctionality === "function") {
    ...
    typeof window.setupEntryHoverFunctionality === "function" &&
    newTimeEntryEl
  ) {
...
window.recalculateEntryOpacities = function () {
  ...
  if (allEntries.length > 0) {
    ...
    if (firstEntryRect.top > mainContentRect.top + 5) {
      // Small buffer
...
function preventPageScroll() {
  ...
  if (mainContent) {
    ...
    mainContent.setAttribute(
      "data-saved-scroll",
      savedMainContentScrollPosition,
    );
...
function restorePageScroll() {
  ...
  if (mainContent) {
    ...
    const savedPosition =
      savedMainContentScrollPosition ||
      parseInt(mainContent.getAttribute("data-saved-scroll")) ||
      0;
